...
class SeedWordAdapter : ChipsAdapter {
    ...
    constructor(existingAdapter: ChipsAdapter) : super(
        existingAdapter.mDataSource,
        existingAdapter.mEditText,
        existingAdapter.mOptions
    )
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (getItemViewType(position) == CHIP) { // Chips
            ...
            mEditText.hint = if (size < 3) {
                ...
                val ordinal = when (size) {
                    2 -> "3rd"; 1 -> "2nd"; else -> "1st"
                }
